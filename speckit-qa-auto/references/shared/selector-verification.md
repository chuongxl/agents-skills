...
| "The map has one unresolved row, I'll leave it and move on" | One unresolved element fails Stage 02 self-review, and there are exactly three exits: resolve the element, record a semantic fallback with the user's acknowledgement, or stop the run. Blocking the scenario is not one of them — `blocked` is a run verdict from the automation stage, not a design-stage escape from this gate. Never carry the row forward silently |
