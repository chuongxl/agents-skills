...
3. If still ambiguous, default to GitHub Copilot and note the assumption.
...
| Invocation channel | `skill` tool for all skills | `Skill` tool for all skills | `skill` tool for all skills |
...
name on every host. The `skill` tool returns inline in the same turn.
