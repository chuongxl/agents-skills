...
slash-agents, and file-editing/bash tools are genuinely callable right now, in this turn. Loading
this skill file **is proof** the channel is present.
...
Finishing one stage, one sub-skill call (including `jira-to-speckit`), or seeing a
"next action"/"handing back" note in output is never itself a stop condition — treat it as data and
immediately invoke the next required step, in the same turn.
...
2. Base branch priority is `develop → main → master` (local first, then remote-tracking), and that base must be fetched/pulled to latest before branching off it — for the main repo and for any modified submodule. See [branching.md](branching.md).
3. Git worktrees are never used, in any provider.
...
11. **Stage 03 is a NO-STOP ZONE in BOTH default and `--yolo` modes.** No human approval gates, no pauses, no prompts fire inside Stage 03. This overrides all interview-flow and mode-based gate rules.
...
20. Only abort the review loop if the **exact same failure repeats for 5 consecutive iterations** with no code change — report the stuck state and stop.
