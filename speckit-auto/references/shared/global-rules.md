...
25. **Run-state persistence is mandatory.** After each stage completes (01, 02, 03), save the current pipeline state — including `worktree_path`, `project_context`, `integration`, `mode`, `current_stage`, and artifact paths — to `<worktree_path>/.speckit/run-state.json`. On re-invocation, if a run-state file exists and its `worktree_path` matches a linked worktree, load it and resume at the stored stage instead of re-running Stage 01. See [run-state.md](run-state.md).
