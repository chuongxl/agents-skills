...
| [jira-to-speckit](./jira-to-speckit/README.md) | Jira-to-spec reader. Fetches a Jira issue, compacts it into a size-bounded Speckit-ready brief, and optionally writes a full-fidelity ticket snapshot for traceability. Optionally exports the Xray tests covering the issue (Cucumber `.feature` plus a manual-tests table) via `xray_tests`. Does not run Speckit stages itself. | `.github/skills/` or `~/.agents/skills/` | GitHub Copilot, Claude, Local | Jira key (e.g., `DDM-1234`), Jira URL, `--issue <url>` | v0.3.0 / Alex Nguyen |
