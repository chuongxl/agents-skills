...
| [jira-to-speckit](./jira-to-speckit/README.md) | Jira-to-spec reader. Fetches a Jira issue, compacts it into a size-bounded Speckit-ready brief, and optionally writes a full-fidelity ticket snapshot for traceability. Optionally exports the Xray tests covering the issue (Cucumber `.feature` plus a manual-tests table carrying verbatim Xray steps) via `xray_tests`. Does not run Speckit stages itself. | `.github/skills/` or `~/.agents/skills/` | GitHub Copilot, Claude, Local | Jira key (e.g., `DDM-1234`), Jira URL, `--issue <url>` | v0.4.0 / Alex Nguyen |
| [speckit-qa-auto](./speckit-qa-auto/README.md) | Jira-to-tests QA pipeline. Sweeps linked issues and existing Xray tests, designs BDD scenarios, converts Manual test cases, verifies selectors against evidence, generates Playwright-BDD tests, runs them with a bounded fix loop, then reports for human review. Bootstraps a test framework when the repo has none. Gherkin is the single artifact for manual and automated testing alike. | `.github/skills/` or `~/.agents/skills/` | GitHub Copilot, Claude, Local | `--issue <story\|epic\|test-key>`, `--impact "<flow>[, <flow>]"`, `--design-only`, `--full-suite`, `--parallel-worktree`, `--pr` | v0.4.0 / Alex Nguyen |
