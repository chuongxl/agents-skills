...
3. If SonarQube MCP configuration exists, run SonarQube scan and collect code-quality findings; prioritize high-severity SonarQube findings in `code issues`.
4. If SonarQube MCP configuration does not exist or scan cannot run, continue with manual review (not a failure condition).
5. Review every changed file line-by-line and every changed class/method body.
6. Merge findings from SonarQube (if available) and manual review into one consolidated result.
